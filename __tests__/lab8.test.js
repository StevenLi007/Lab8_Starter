describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < productItems.length; i++) {
      ...
      const buttonText = await button.evaluate((element) => element.innerText);
      if (buttonText != 'Remove from Cart') { // Implemented so the first item isn't double clicked due to prev test
        await button.click();
      }
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < updatedProductItems.length; i++) {
      const shadowRoot = await updatedProductItems[i].getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      const buttonText = await button.evaluate((element) => element.innerText);
      expect(buttonText).toBe('Remove from Cart');
    }
